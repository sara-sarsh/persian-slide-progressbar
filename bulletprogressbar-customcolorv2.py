...
# -*- coding: utf-8 -*-
"""
Created on Tue Jan 21 02:00:39 2025
    ...
    for chapter_index, slides_in_chapter in enumerate(slides_per_chapter):
        for slide_number in range(1, slides_in_chapter + 1):
            ...
            for idx, chapter_name in enumerate(chapter_names):
                ...
                text_x = chapter_center_x - (text_width // 2)

                # Highlight current chapter name
                if slide_count > sum(slides_per_chapter[:idx]) and slide_count <= sum(slides_per_chapter[:idx + 1]):
                    text_color = "#53585F"  # Highlight color
                else:
                    text_color = "#8F95A3"  # Faded color

                draw.text((text_x, y_bullet - circle_diameter - 50), reshaped_name, fill=text_color, font=font)
